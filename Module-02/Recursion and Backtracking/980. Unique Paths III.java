class Solution {
    ...
    private int helper(int row, int col, int m, int n, int cnt, int grid[][]) {
        ...
        ans += helper(row-1, col, m, n, cnt-1, grid); // top direction
        ans += helper(row, col-1, m, n, cnt-1, grid); // left direction
        ans += helper(row+1, col, m, n, cnt-1, grid); // down direction
        ans += helper(row, col+1, m, n, cnt-1, grid); // right direction
